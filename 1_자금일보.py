...
# 엑셀 파일 자동 로드 (사용자 업로드 없이)
EXCEL_FILE = "ACOT_CashFlow_TEST_classified_완료"

try:
    df_daily = pd.read_excel(EXCEL_FILE, sheet_name="Daily")
    df_daily['지출일'] = pd.to_datetime(df_daily['지출일'])
except FileNotRoundError:
    st.error(f"'{EXCEL_FILE}' 파일을 찾을 수 없습니다. GitHub 저장소에 올려주세요.")
except Exception as e:
    st.error(f"엑셀 파일을 읽은 중 오류가 발생했습니다: {e}")
